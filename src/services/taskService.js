...
const taskService = {
    ...
    update(id, data) {
        const { assigned_to, title, description, tag, priority, status, deadline } = data;
        return new Promise((resolve, reject) => {
            db.run(
                `UPDATE tasks SET assigned_to=?, title=?, description=?, tag=?, priority=?, status=?, deadline=? WHERE id=?`,
                [assigned_to, title, description, tag, priority, status, deadline, id],
                function (err) {
                    if (err) reject(err);
